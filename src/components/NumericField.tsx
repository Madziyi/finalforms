...
export function NumericField({ field, value, history, onChange, onTrend, historyMode = "daily", disabled = false, allowCalculatedEdits = false, pending = false }: {
  ...
  return <div className={`numeric-row ${warn ? "outside" : ""} ${showHistory ? "" : "no-history"}`} style={showHistory ? undefined : { gridTemplateColumns: "minmax(220px, 1.1fr) minmax(170px, .75fr)" }}>
    ...
    <div className="entry-cell"><span>Entry</span><div className="input-with-unit">{pending ? <span className="history-empty">Waiting for daily totals</span> : <><FormattedNumberInput value={numeric} disabled={disabled || field.calculated && !allowCalculatedEdits} onChange={onChange} aria-label={field.label} />{field.unit && <span className="unit-suffix">{field.unit}</span>}</>}</div>{warn && <span className="range-warning"><TriangleAlert size={14} /> Outside target</span>}</div>
