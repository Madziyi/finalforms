...
export type LocalEntry = {
  ...
  /** Previous canonical ID when a completed record was moved to a new context. */
  movedFromId?:string|null;
  /** Server revision this local snapshot is based on; null means a new record. */
...
function normalizedEntry(entry:LocalEntry,updatedAt=new Date().toISOString()):LocalEntry{
  ...
  if(!next.contextKey) throw new Error("Completed entries require a complete normalized context.");
  const canonicalId=canonicalRecordId(next.formKey,next.context);
  if(next.entryId===canonicalId) return next;
  // A local-only identity has never existed in the cloud, so changing its
  // context is not a server move.  Only an acknowledged record carries the
 ...
 * its source acknowledgement arrives after the local context change.
 */
export async function settleCompleted(entryId:string,localVersion:number,acknowledgedRevision?:number){let changed=false;const now=new Date().toISOString();await offlineDb.transaction("rw",offlineDb.completedSync,offlineDb.entries,async()=>{const item=await offlineDb.completedSync.get(entryId);if(item&&item.status==="uploading"&&entryVersion({localVersion:item.payload.localVersion})===localVersion){await offlineDb.completedSync.update(entryId,{status:"synced",lastError:null,nextAttemptAt:null,updatedAt:now});const entry=await offlineDb.entries.get(entryId);if(entry&&entryVersion(entry)===localVersion)await offlineDb.entries.update(entryId,{uploadError:null,baseRevision:acknowledgedRevision??entry.baseRevision});changed=true;}else if(item&&entryVersion({localVersion:item.payload.localVersion})>localVersion&&acknowledgedRevision!=null){await offlineDb.completedSync.update(entryId,{payload:{...item.payload,baseRevision:acknowledgedRevision},updatedAt:now});const entry=await offlineDb.entries.get(entryId);if(entry)await offlineDb.entries.update(entryId,{baseRevision:acknowledgedRevision});changed=true;}
    if(acknowledgedRevision!=null){const pendingMoves=(await offlineDb.completedSync.toArray()).filter(candidate=>candidate.payload.movedFromId===entryId&&["pending","retry"].includes(candidate.status));for(const move of pendingMoves){await offlineDb.completedSync.update(move.entryId,{payload:{...move.payload,baseRevision:acknowledgedRevision},updatedAt:now});await offlineDb.entries.update(move.entryId,{baseRevision:acknowledgedRevision});changed=true;}}
  });if(changed)emit("completed-uploaded",{entryId,localVersion});return changed;}
