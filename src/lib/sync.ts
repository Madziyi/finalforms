...
import { getOperators, listCompletedSync, localEvents, markCompletedRetry, markCompletedSending, migrateV1LocalData, pendingCompletedCount, purgeExpiredLocalData, putOperators, settleCompleted } from "./offlineDb";
...
async function processOne(item:Awaited<ReturnType<typeof listCompletedSync>>[number]){const localVersion=Number.isInteger(item.payload.localVersion)?item.payload.localVersion:0;if(!await markCompletedSending(item.entryId,localVersion))return;try{await uploadCompleted(item.payload);await settleCompleted(item.entryId,localVersion);publish({lastSuccess:new Date().toISOString(),lastError:null});}catch(error){const message=error instanceof ApiError?error.message:error instanceof Error?error.message:String(error);await markCompletedRetry(item.entryId,localVersion,message);publish({lastError:`Upload failed: ${message}`});}}
async function replay(){if(!navigator.onLine||replaying)return;replaying=true;publish({running:true});try{const items=(await listCompletedSync()).filter(eligible).sort((a,b)=>a.updatedAt.localeCompare(b.updatedAt));for(const item of items.slice(0,8))await processOne(item);await refreshCount();}finally{replaying=false;publish({running:false});}}
export async function syncNow(){if(!navigator.onLine){await refreshCount();return;}await replay();}
async function compatibilityCheck(){try{const meta=await getMeta();publish({compatibility:meta.protocolVersion===2?"compatible":"update_required"});}catch(error){if(navigator.onLine)publish({lastError:error instanceof Error?error.message:String(error)});}}
