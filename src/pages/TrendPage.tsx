...
type PointRange = 7 | 30 | 90 | 200;

const POINT_RANGES: Array<{ value: PointRange; label: string }> = [
  { value: 7, label: "7 points" },
  { value: 30, label: "30 points" },
  { value: 90, label: "90 points" },
  { value: 200, label: "All available · up to 200" },
];
...
function TrendChart({ points, field }: { points: TrendPoint[]; field: FieldDefinition }) {
  ...
  const renderAxisTick = (props: any) => (
    <TrendAxisTick {...props} chartData={chartData} />
  );
    ...
    <div className="trend-chart-shell">
      <div className="trend-chart-frame" role="img" aria-label={`${field.label} historical readings chart`}>
        <ResponsiveContainer width="100%" height={430}>
          <LineChart data={chartData} margin={{ top: 18, right: 26, left: 4, bottom: 22 }}>
            <CartesianGrid stroke="#dfe4ec" strokeDasharray="4 4" vertical={false} />
            <XAxis
              dataKey="label"
              height={54}
              interval="preserveStartEnd"
              tick={(props) => renderAxisTick(props)}
      ...
      {chartData.length === 1 && (
        <p className="trend-chart-note">One published point is available. A line will appear after another reading is recorded.</p>
      )}
    </div>
  );
}
...
export function TrendPage() {
  const { formKey = "", fieldKey } = useParams();
  ...
  useEffect(() => {
    ...
    setMessage(null);
    setPoints([]);
    void getTrend(formKey, selectedFieldKey, pointRange)
      .then((result) => { if (!cancelled) setPoints(result.points); })
      .catch((reason: unknown) => { if (!cancelled) setMessage(reason instanceof Error ? reason.message : String(reason)); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [formKey, pointRange, retryCount, selectedFieldKey]);
  ...
  const onFieldChange = (nextFieldKey: string) => {
    ...
    navigate(`/trends/${formKey}/${nextFieldKey}`, { replace: true });
  };
  ...
  return (
    <div className="page-stack trend-page">
      ...
      <section className="trend-controls card-surface" aria-label="Trend controls">
        ...
        <label><span>Points</span><select aria-label="Number of trend points" value={pointRange} onChange={(event) => setPointRange(Number(event.target.value) as PointRange)}>{POINT_RANGES.map((range) => <option key={range.value} value={range.value}>{range.value === 200 ? "All available" : range.value}</option>)}</select></label>
      </section>
