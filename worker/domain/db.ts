...
export async function openAttention(db: D1Database, input: {
  ...
  await db.prepare(`INSERT INTO attention_items(attention_id,aggregate_id,plant_date,category,code,severity,message,details_json) VALUES(?,?,?,?,?,?,?,?)`)
...
export async function resolveAttention(db: D1Database, aggregateId: string, codes?: string[]) {
  if (codes?.length) {
    ...
    await db.prepare(`UPDATE attention_items SET resolved_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE aggregate_id=? AND resolved_at IS NULL AND code IN (${placeholders})`).bind(aggregateId, ...codes).run();
  } else {
    await db.prepare(`UPDATE attention_items SET resolved_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE aggregate_id=? AND resolved_at IS NULL`).bind(aggregateId).run();
  }
}
