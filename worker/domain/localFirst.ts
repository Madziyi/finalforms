...
export async function upsertCompletedRecord(db: D1Database, input: unknown): Promise<any> {
  ...
  const result: SyncReceipt & { affectedDates: string[] } = { syncId: payload.syncId!, aggregateId: record.aggregateId, outcome: "accepted", revision: nextRevision, record: savedRecord, affectedDates: [...affectedDates].sort() };
  const statements: D1PreparedStatement[] = [];
